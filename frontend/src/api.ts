...
export const api = {
  ...
  getAgentJob: (jobId: number, projectId: number) => request<AgentJob>(`/api/agent-jobs/${jobId}?project_id=${projectId}`),
  pauseAgentJob: (jobId: number, projectId: number) => request<AgentJob>(`/api/agent-jobs/${jobId}/pause`, json({ project_id: projectId })),
  resumeAgentJob: (jobId: number, projectId: number) => request<AgentJob>(`/api/agent-jobs/${jobId}/resume`, json({ project_id: projectId })),
  retryAgentJob: (jobId: number, projectId: number) => request<AgentJob>(`/api/agent-jobs/${jobId}/retry`, json({ project_id: projectId })),
  cancelAgentJob: (jobId: number, projectId: number) => request<AgentJob>(`/api/agent-jobs/${jobId}/cancel`, json({ project_id: projectId })),
  ...
  getAiSettings: () => request<{ configured: boolean; base_url: string | null; model: string | null; provider: string | null; providers?: Record<string, { configured: boolean; base_url: string | null; model: string | null }>; assignments?: { keyword_review: string; title_generation: string; content_generation?: string } }>("/api/settings/ai"),
  saveAiSettings: (body: object) => request<{ configured: boolean; base_url: string; model: string; provider: string; providers?: Record<string, { configured: boolean; base_url: string | null; model: string | null }>; assignments?: { keyword_review: string; title_generation: string; content_generation?: string } }>("/api/settings/ai", json(body)),
  ...
  learnFromPublishedGscContent: (projectId: number, body: { days?: number } = {}) => request<{ job_id: number; project_id: number; status: "queued"; workflow_version: "gsc-feedback-v1"; message: string }>(`/api/projects/${projectId}/gsc/learn-content`, json(body)),
  ...
  previewContentPrompt: (assetId: number, body: object) => request<ContentPromptPreview>(`/api/content-assets/${assetId}/prompt-preview`, json(body)),
  previewCompetitors: (assetId: number, body: object) => request<import("./types").CompetitorResearchPreview>(`/api/content-assets/${assetId}/preview-competitors`, json(body)),
  previewCompetitorOutline: (assetId: number, body: object) => request<import("./types").CompetitorOutlinePreview>(`/api/content-assets/${assetId}/preview-outline`, json(body)),
  previewContent: (assetId: number, body: object) => request<{ heading: string; markdown: string; source_count: number; provider: string; model?: string | null; persisted: false }>(`/api/content-assets/${assetId}/preview-content`, json(body)),
  restartContentAsset: (assetId: number, projectId: number) => request<ContentAssetDetail>(`/api/content-assets/${assetId}/restart`, json({ project_id: projectId, confirm_reset: true })),
  ...
  listCompetitorUrlCatalog: (projectId: number) => request<CompetitorUrlCatalogItem[]>(`/api/competitor-url-catalog?project_id=${projectId}`),
  collectCompetitorUrlCatalog: (projectId: number) => request<CompetitorCatalogCollectionRun>("/api/competitor-url-catalog/collect", json({ project_id: projectId })),
  listCompetitorCatalogCollectionRuns: (projectId: number) => request<CompetitorCatalogCollectionRun[]>(`/api/competitor-url-catalog/collection-runs?project_id=${projectId}`),
  listCompetitorCatalogCollectionRunItems: (projectId: number, runId: number) => request<import("./types").CollectionRunItem[]>(`/api/competitor-url-catalog/collection-runs/${runId}/items?project_id=${projectId}`),
  listCollectionPlans: (projectId: number) => request<import("./types").CollectionPlan[]>(`/api/collection-plans?project_id=${projectId}`),
  saveCollectionPlan: (body: { project_id: number; source_type: "domain" | "keyword" | "first_party"; source_value: string; enabled?: boolean; schedule?: string; settings?: Record<string, unknown> }) => request<import("./types").CollectionPlan>("/api/collection-plans", json(body)),
  discoverCollectionPlan: (projectId: number, planId: number, body: { locale?: string; max_results?: number } = {}) => request<{ plan_id: number; project_id: number; query: string; discovered_count: number; queued_count: number; excluded_count: number }>(`/api/collection-plans/${planId}/discover`, json({ project_id: projectId, ...body })),
  learnFromCollectedCompetitorContent: (projectId: number, body: { provider?: "openai" | "gemini" | "deepseek"; model?: string } = {}) => request<CollectedCompetitorContentLearningRun>("/api/competitor-content-learning/learn", json({ project_id: projectId, ...body })),
  listCollectedCompetitorContentLearningRuns: (projectId: number) => request<CollectedCompetitorContentLearningRun[]>(`/api/competitor-content-learning/runs?project_id=${projectId}`),
